...
class AnalyticsServiceFirebase : AnalyticsService {
    ...
    override fun setUserCountry(countryCode: String?) {
        countryCode?.let {
            analytics?.setUserProperty("country", it.lowercase(Locale.US))
            crashlytics?.setCustomKey("country", it.lowercase(Locale.US))
        }
    }
}
